...
public final class FileWrapper {
    ...
    public static String getColumn(String file,int row,int col) {
        Map<String, FileWrapper> my = filePacks.get();
        FileWrapper fw = my.get(file);
        ...
        return fw.getColumn(row,col);
    }

    private String getColumn(int row,int col) {
        if (currentRow == NO_LINE) {
            currentRow = container.nextRow();
        }
        return container.getColumn(row, col);
    }
